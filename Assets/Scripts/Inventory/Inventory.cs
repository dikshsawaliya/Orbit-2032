...
public class Inventory : ScriptableObject
{
    ...
    public InventoryUI inventoryUI
    {
        get
        {
            if (!_inventoryUI)
            {
                _inventoryUI = Instantiate(inventoryUIPrefab, playerEquipmnet.GetUIParent());
            }
    ...
    private PlayerEquipmentController playerEquipmnet;

    public void InitInventory(PlayerEquipmentController playerEquipmnet)
    {
        this.playerEquipmnet = playerEquipmnet;
    ...
    public void RemoveItem(InventoryItem item, int count)
    {
        ...
        {
            Debug.Log(string.Format("Cant Remove {0}.This item is not in inventory"));
        }
    }
}
